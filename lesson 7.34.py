...
def Counting_vowels(text):
    ...
    for s in text:
      if s.upper() in list_vowels:
        count += 1
    return count
list_vowels = "АЕЁИОНЫЭЮЯ"   # все возможные гласные буквы
phrase = 'пара-ра-рама рам-пам-папам па-ра-па-да '
vowels_in_text = list(map(Counting_vowels,phrase.split())) # делаем список, который содержит колиество гласный букв
if all(x == vowels_in_text[0] for x in vowels_in_text):
  print("Парам пам-пам")
else:
  print("Пам парам")
